...
aso_comercial AS (
    ...
    FROM dcac.sociedades_tags FINAL AS st
...
repre_vinc AS (
    ...
    FROM dcac.rel_usuarios_sociedades FINAL AS RUS
...
RepreCompraInvernada AS (
    ...
    FROM dcac.lotes_x_interesados FINAL AS lxi LEFT JOIN dcac.usuarios AS u ON u.usuario = lxi.representante_de_compra GROUP BY lxi.revisacion
),
RepreVentaInv AS (
    SELECT r.revisacion AS id, arrayStringConcat(groupUniqArray(concat(u.nombre, ' ', u.apellido)), ', ') AS RepresentanteVenta
    FROM dcac.revisaciones FINAL AS r LEFT JOIN dcac.usuarios AS u ON u.usuario = r.representante GROUP BY r.revisacion
),
...
c5_datitos AS (
    ...
    FROM dcac.revisaciones FINAL AS r INNER JOIN dcac.sociedades_tags AS sv ON r.sociedad_vendedora = sv.id
    ...
    FROM dcac.negocios FINAL AS n INNER JOIN dcac.sociedades_tags AS sv ON n.sociedad_vendedora = sv.id
    WHERE toString(sv.razon_social) != ''
),
c5_ranked AS (
...
cg_datitos AS (
    ...
    FROM dcac.revisaciones FINAL AS r INNER JOIN dcac.sociedades_tags AS sv ON r.sociedad_vendedora = sv.id WHERE toString(sv.razon_social) != ''
    UNION ALL 
    ...
    FROM dcac.negocios FINAL AS n INNER JOIN dcac.sociedades_tags AS sv ON n.sociedad_vendedora = sv.id WHERE toString(sv.razon_social) != ''
),
concrecion_gral AS (
...
cte_estados_inv AS (
    ...
    FROM dcac.revisaciones FINAL AS RS
...
Estados_MAG AS (
    SELECT n.id AS ID_Negocio,
        CASE 
            ...
            ELSE '0'
        END AS ESTADO
    FROM negocios.liquidacion_oficial FINAL AS lo
