/**
 * Vercel Serverless API - Reporte Semanal Híbrido 5.1 (Direct Auth)
...
function getSheetsApi() {
    if (!GOOGLE_EMAIL || !GOOGLE_KEY) return null;
    try {
...
module.exports = async function handler(req, res) {
    ...
    try {
        if (op === "config") {
            ...
            if (!api || !SPREADSHEET_ID) {
                return res.status(500).json({ error: "Faltan variables: GOOGLE_SERVICE_ACCOUNT_EMAIL, GOOGLE_PRIVATE_KEY o GOOGLE_SHEET_ID" });
            }
            ...
            const api = getSheetsApi();
            if (!api || !SPREADSHEET_ID) return res.status(500).json({ error: "Google Sheets no configurado" });
...
function processReport(baseRows, opsRows, vR, ac, startTs, endTs) {
    ...
    const dCom = vR[0].values || [];
    const dAge = vR[1].values || [];
    const dLeads = vR[2].values || [];
    const dAux = vR[3].values || [];
    const dSac = vR[4].values || [];
    const dRem = vR[5].values || [];
    ...
    for (let row of dCom) {
        ...
        const fs = formatDateYMD(f);
        if (fs >= dInicio && fs <= dFin) { if (String(row[7] || "").trim() === "") r.com++; if (row[0]) sG[row[0]] = 1; }
    ...
    for (let row of dLeads) {
        ...
        const fs = formatDateYMD(f);
        if (fs >= dInicio && fs <= dFin) r.nuevas++;
        if (fs >= dInicioAnt && fs <= dFinAnt) r.pNuevas++;
    }

    const ssgAll = [];
    for (let row of dAux) {
        if (String(row[1] || "").trim() !== ac) continue;
        const isN = (String(row[4] || "").trim().toUpperCase() === "NUEVO");
        if (isN) r.socSinGestNum++;
        const fd = parseSheetDate(row[2]);
        const obj = { kt: row[31] || "", kv: row[36] || "", soc: row[29] || "", fa: fd ? fmtDate(fd) : String(row[2] || ""), fu: row[37] || "", ug: row[40] || "", ua: row[39] || "", sg: row[38] || "", w: Number(row[22]) || 0, cDateStr: formatDateYMD(fd) };
        if (isN) ssgAll.push(obj);
        if (obj.cDateStr >= dInicio && obj.cDateStr <= dFin) { if (Number(row[32]) <= 6 && String(row[40] || "").toLowerCase() !== "sin gestión") r.actSemanal.push(obj); }
    }
    ssgAll.sort((a, b) => b.w - a.w); r.ssgTop5 = ssgAll.slice(0, 5);

    for (let row of dSac) {
        if (String(row[18] || "").trim() !== ac) continue;
        const f = parseSheetDate(row[19]); if (!f) continue;
        const fs = formatDateYMD(f);
        if (fs >= dInicio && fs <= dFin) r.sacs.push({ s: row[1] || "", f: f.getTime(), e: row[3] || "" });
    ...
    for (let i = 0; i < dRem.length; i++) {
        ...
        const fs = formatDateYMD(f);
        const id = row[3] || i;
        if (fs >= dInicio && fs <= dFin) remIds[id] = 1;
        if (fs >= dInicioAnt && fs <= dFinAnt) pRemIds[id] = 1;
    }
