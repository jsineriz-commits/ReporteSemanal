...
aso_comercial AS (
    ...
    FROM dcac.sociedades_tags AS st
...
repre_vinc AS (
    ...
    FROM dcac.rel_usuarios_sociedades AS RUS
...
RepreCompraInvernada AS (
    ...
    FROM dcac.lotes_x_interesados AS lxi LEFT JOIN dcac.usuarios AS u ON u.usuario = lxi.representante_de_compra GROUP BY lxi.revisacion
),
RepreVentaInv AS (
    SELECT r.revisacion AS id, arrayStringConcat(groupUniqArray(concat(u.nombre, ' ', u.apellido)), ', ') AS RepresentanteVenta
    FROM dcac.revisaciones AS r LEFT JOIN dcac.usuarios AS u ON u.usuario = r.representante GROUP BY r.revisacion
),
...
liq_uniq AS (SELECT * FROM negocios.liquidaciones ORDER BY toFloat64OrZero(toString(kg_salida_neto)) DESC LIMIT 1 BY negocio),
dc_uniq AS (SELECT * FROM dcac.detalles_carga ORDER BY toFloat64OrZero(toString(peso_neto)) DESC LIMIT 1 BY revisacion),
lxi_uniq AS (SELECT * FROM dcac.lotes_x_interesados LIMIT 1 BY revisacion),
ar_neg_uniq AS (SELECT * FROM dcac.analisis_resultados LIMIT 1 BY negocio),
ar_rev_uniq AS (SELECT * FROM dcac.analisis_resultados LIMIT 1 BY revisacion),
arp_neg_uniq AS (SELECT * FROM dcac.analisis_resultados_proyectado LIMIT 1 BY negocio),
arp_rev_uniq AS (SELECT * FROM dcac.analisis_resultados_proyectado LIMIT 1 BY revisacion),
ib_neg_uniq AS (SELECT * FROM dcac.informes_baja LIMIT 1 BY negocio),
ib_rev_uniq AS (SELECT * FROM dcac.informes_baja LIMIT 1 BY revisacion),
...
c5_datitos AS (
    ...
    FROM dcac.revisaciones AS r INNER JOIN dcac.sociedades_tags AS sv ON r.sociedad_vendedora = sv.id
    ...
    FROM dcac.negocios AS n INNER JOIN dcac.sociedades_tags AS sv ON n.sociedad_vendedora = sv.id
    WHERE toString(sv.razon_social) != ''
),
c5_ranked AS (
...
cg_datitos AS (
    ...
    FROM dcac.revisaciones AS r INNER JOIN dcac.sociedades_tags AS sv ON r.sociedad_vendedora = sv.id WHERE toString(sv.razon_social) != ''
    UNION ALL 
    ...
    FROM dcac.negocios AS n INNER JOIN dcac.sociedades_tags AS sv ON n.sociedad_vendedora = sv.id WHERE toString(sv.razon_social) != ''
),
concrecion_gral AS (
...
FechaFaenaReal AS (
    ...
    FROM negocios.liquidacion_oficial WHERE lo_tipo_liquid = 'interna' GROUP BY lo_negocio
),
Estados_FAE AS (
...
cte_estados_inv AS (
    SELECT RS.revisacion AS ID_Revisacion, 
        CASE
            ...
            WHEN toString(RS.estado) = '4' AND toString(RS.estado_b) = '6' AND toString(DTC.fecha_carga_final) NOT IN ('','0000-00-00') AND toString(RS.no_concretado) = '0' AND ( ((AR.fecha_pago_real_1 IS NULL OR toString(AR.fecha_pago_real_1) IN ('','0000-00-00')) AND today() > addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c1)))) OR ((AR.fecha_pago_real_1 IS NOT NULL AND toString(AR.fecha_pago_real_1) NOT IN ('','0000-00-00')) AND toInt32OrZero(toString(DTC.plazo_c2)) > 0 AND (AR.fecha_pago_real_2 IS NULL OR toString(AR.fecha_pago_real_2) IN ('','0000-00-00')) AND today() > addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c2)))) OR ((AR.fecha_pago_real_2 IS NOT NULL AND toString(AR.fecha_pago_real_2) NOT IN ('','0000-00-00')) AND toInt32OrZero(toString(DTC.plazo_c3)) > 0 AND (AR.fecha_pago_real_3 IS NULL OR toString(AR.fecha_pago_real_3) IN ('','0000-00-00')) AND today() > addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c3)))) OR ((AR.fecha_pago_real_3 IS NOT NULL AND toString(AR.fecha_pago_real_3) NOT IN ('','0000-00-00')) AND toInt32OrZero(toString(DTC.plazo_c4)) > 0 AND (AR.fecha_pago_real_4 IS NULL OR toString(AR.fecha_pago_real_4) IN ('','0000-00-00')) AND today() > addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c4)))) ) THEN 'Pagos Vencidos'
            ...
            WHEN toString(RS.estado) = '4' AND toString(RS.estado_b) = '6' AND toString(RS.no_concretado) = '0' AND ( ((AR.fecha_pago_real_1 IS NULL OR toString(AR.fecha_pago_real_1) IN ('','0000-00-00')) AND today() <= addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c1)))) OR ((AR.fecha_pago_real_1 IS NOT NULL AND toString(AR.fecha_pago_real_1) NOT IN ('','0000-00-00')) AND toInt32OrZero(toString(DTC.plazo_c2)) > 0 AND (AR.fecha_pago_real_2 IS NULL OR toString(AR.fecha_pago_real_2) IN ('','0000-00-00')) AND today() <= addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c2)))) OR ((AR.fecha_pago_real_2 IS NOT NULL AND toString(AR.fecha_pago_real_2) NOT IN ('','0000-00-00')) AND toInt32OrZero(toString(DTC.plazo_c3)) > 0 AND (AR.fecha_pago_real_3 IS NULL OR toString(AR.fecha_pago_real_3) IN ('','0000-00-00')) AND today() <= addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c3)))) OR ((AR.fecha_pago_real_3 IS NOT NULL AND toString(AR.fecha_pago_real_3) NOT IN ('','0000-00-00')) AND toInt32OrZero(toString(DTC.plazo_c4)) > 0 AND (AR.fecha_pago_real_4 IS NULL OR toString(AR.fecha_pago_real_4) IN ('','0000-00-00')) AND today() <= addDays(toDateOrNull(toString(DTC.fecha_carga_final)), toInt32OrZero(toString(DTC.plazo_c4)))) ) THEN 'Cerradas'
            WHEN toString(RS.estado) = '4' AND toString(RS.estado_b) = '6' AND toString(RS.no_concretado) = '0' AND ( (((toInt32OrZero(toString(DTC.plazo_c1)) != 0 AND toInt32OrZero(toString(DTC.plazo_c2)) != 0 AND toInt32OrZero(toString(DTC.plazo_c3)) != 0 AND toInt32OrZero(toString(DTC.plazo_c4)) != 0) OR (toInt32OrZero(toString(DTC.plazo_c1)) = 0 AND toInt32OrZero(toString(DTC.plazo_c2)) != 0 AND toInt32OrZero(toString(DTC.plazo_c3)) != 0 AND toInt32OrZero(toString(DTC.plazo_c4)) != 0  AND toInt32OrZero(toString(DTC.plazo_c1_p)) > 0) AND (AR.fecha_pago_real_1 IS NOT NULL AND toString(AR.fecha_pago_real_1) NOT IN ('','0000-00-00')) AND (AR.fecha_pago_real_2 IS NOT NULL AND toString(AR.fecha_pago_real_2) NOT IN ('','0000-00-00')) AND (AR.fecha_pago_real_3 IS NOT NULL AND toString(AR.fecha_pago_real_3) NOT IN ('','0000-00-00')) AND (AR.fecha_pago_real_4 IS NOT NULL AND toString(AR.fecha_pago_real_4) NOT IN ('','0000-00-00')))) OR ((toInt32OrZero(toString(DTC.plazo_c1)) != 0 AND toInt32OrZero(toString(DTC.plazo_c2)) != 0 AND toInt32OrZero(toString(DTC.plazo_c3)) != 0) OR (toInt32OrZero(toString(DTC.plazo_c1)) = 0 AND toInt32OrZero(toString(DTC.plazo_c2)) != 0 AND toInt32OrZero(toString(DTC.plazo_c3)) != 0 AND toInt32OrZero(toString(DTC.plazo_c1_p)) > 0) AND (AR.fecha_pago_real_1 IS NOT NULL AND toString(AR.fecha_pago_real_1) NOT IN ('','0000-00-00')) AND (AR.fecha_pago_real_2 IS NOT NULL AND toString(AR.fecha_pago_real_2) NOT IN ('','0000-00-00')) AND (AR.fecha_pago_real_3 IS NOT NULL AND toString(AR.fecha_pago_real_3) NOT IN ('','0000-00-00'))) OR ((toInt32OrZero(toString(DTC.plazo_c1)) != 0 AND toInt32OrZero(toString(DTC.plazo_c2)) != 0) OR (toInt32OrZero(toString(DTC.plazo_c1)) = 0 AND toInt32OrZero(toString(DTC.plazo_c2)) != 0 AND toInt32OrZero(toString(DTC.plazo_c1_p)) > 0) AND (AR.fecha_pago_real_1 IS NOT NULL AND toString(AR.fecha_pago_real_1) NOT IN ('','0000-00-00')) AND (AR.fecha_pago_real_2 IS NOT NULL AND toString(AR.fecha_pago_real_2) NOT IN ('','0000-00-00'))) OR ((toInt32OrZero(toString(DTC.plazo_c1)) = 0 AND toInt32OrZero(toString(DTC.plazo_c1_p)) > 0) AND (AR.fecha_pago_real_1 IS NOT NULL AND toString(AR.fecha_pago_real_1) NOT IN ('','0000-00-00'))) OR ((toInt32OrZero(toString(DTC.plazo_c1)) != 0) AND (AR.fecha_pago_real_1 IS NOT NULL AND toString(AR.fecha_pago_real_1) NOT IN ('','0000-00-00'))) ) THEN 'Negocios Terminados'
...
-- =================================================================
faena_base AS (
    SELECT * FROM dcac.negocios WHERE toDateOrNull(toString(fecha)) >= '2024-01-01' AND toInt64OrZero(toString(id)) > 0
),
invernada_base AS (
    SELECT * FROM dcac.revisaciones WHERE toDateOrNull(toString(fecha_hora)) >= '2024-01-01' AND toInt64OrZero(toString(revisacion)) > 0
),
...
faena_final AS (
    ...
    LEFT JOIN (SELECT * FROM negocios.compra_inmediata_faena LIMIT 1 BY negocio) AS ci ON n.id = ci.negocio AND toString(ci.comprada) = '1'
    LEFT JOIN dcac.usuarios AS us_o ON n.generado_por = us_o.usuario AND toString(us_o.perfil) = '3' 
    LEFT JOIN (SELECT * FROM negocios.cd_logs_estado LIMIT 1 BY cd_cotizacion_id) AS cdl ON cd_cotizaciones.id = cdl.cd_cotizacion_id AND toString(cdl.estado) = '1'
