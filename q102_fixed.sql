...
lo_uniq AS (SELECT lo_negocio, lo_tipo_liquid, lo_fecha_faena_real, lo_fecha_pago_real FROM negocios.liquidacion_oficial WHERE lo_tipo_liquid = 'interna' LIMIT 1 BY lo_negocio),
...
-- =================================================================
faena_base AS (
    SELECT * FROM dcac.negocios WHERE toDateOrNull(toString(fecha)) >= '2026-01-01' AND toInt64OrZero(toString(id)) > 0
),
invernada_base AS (
    SELECT * FROM dcac.revisaciones WHERE toDateOrNull(toString(fecha_hora)) >= '2026-01-01' AND toInt64OrZero(toString(revisacion)) > 0
),
